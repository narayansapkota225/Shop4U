...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($img_name) {
        ...
        if (in_array($ext, $allowedfileExtensions)) {
            ...
            if (move_uploaded_file($srcpath, $despath)) {
                ...
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    ...
                    mysqli_stmt_bind_param($stmt, "ssdsiii", $title, $desc, $price, $newname, $cat, $feature, $active);
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            mysqli_stmt_bind_param($stmt, "ssdsiii", $title, $desc, $price, $img_name, $cat, $feature, $active);
            mysqli_stmt_execute($stmt);
            header("Location:../admin/product.php? result=Product has been Successfully added");
        }
    }
